...
Mean2 = 10.0 # Input parameter, mean of second normal  probability distribution
Standard_dev2 = 8.0 #@param {type:"number"}
Mean3 = 6.0 # Input parameter, mean of second normal  probability distribution
Standard_dev3 = 8.0 #@param {type:"number"}

# generate data
y1 = np.random.normal(Mean1, Standard_dev1, 1500)
y2 = np.random.normal(Mean2, Standard_dev2, 1400)
y3 = np.random.normal(Mean3, Standard_dev3, 1300)
...
print('Input Gaussian {:}: μ = {:.2}, σ = {:.2}'.format("3", Mean3, Standard_dev3))
...
class GaussianMixture_self:
    ...
    def Estep(self):
        ...
        for datum in self.data:  
            ...
            wp2 = self.two.pdf(datum) * self.mix
            wp3 = self.three.pdf(datum) * (1. - self.mix)
            # compute denominator
            den = wp1 + wp2 + wp3
            # normalize
            wp1 /= den   
            wp2 /= den
            wp3 /= den # wp1+wp2= 1, it either belongs to gaussian 1 or gaussion 2
            # add into loglike
    ...
    def Mstep(self, weights):
        ...
        (left, rigt, centre) = zip(*weights) 
        one_den = sum(left)
        two_den = sum(rigt)
        three_den = sum(centre)

        # compute new means
        self.one.mu = sum(w * d  for (w, d) in zip(left, data)) / one_den
        self.two.mu = sum(w * d  for (w, d) in zip(rigt, data)) / two_den
        self.three.mu = sum(w * d  for (w, d) in zip(centre, data)) / three_den
        ...
        self.three.sigma = sqrt(sum(w * ((d - self.three.mu) ** 2)
                                  for (w, d) in zip(centre, data)) / three_den)
        # compute new mix
    ...
    def pdf(self, x):
        return (self.mix)*self.one.pdf(x) + (self.mix)*self.two.pdf(x) + (1-self.mix)*self.three.pdf(x)
...
print('Gaussian {:}: μ = {:.2}, σ = {:.2}, weight = {:.2}'.format("2", best_mix.two.mu, best_mix.two.sigma, best_mix.mix))
print('Gaussian {:}: μ = {:.2}, σ = {:.2}, weight = {:.2}'.format("3", best_mix.three.mu, best_mix.three.sigma, (1-best_mix.mix)))
#Show mixture
sns.distplot(data, bins=20, kde=False, norm_hist=True);
...
g_right = [best_mix.two.pdf(e) * best_mix.mix for e in x]
plt.plot(x, g_right, label='gaussian two');
g_centre = [best_mix.three.pdf(e) * (1-best_mix.mix) for e in x]
plt.plot(x, g_centre, label='gaussian three');
plt.legend();
